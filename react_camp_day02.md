...
- 컴포넌트를 역할에 따라 Container와 Component로 분리하여 스케일이 커져도 문제 없는 구조를 만들자.
  - Container는 store를 가져가는 컴포넌트
  - Component는 store를 가지지 않는 컴포넌트
- 컴포넌트를 분리를 잘하려면 경험이 많이 필요하다. 경험이 없을때 실력을 향상시키고 싶으면 레퍼런스를 많이 봐야한다.
  - antd 라이브러리가 컴포넌트 분리가 잘되어있으니 참고하면 좋을 것이다.
